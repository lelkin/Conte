/*
 * Adapted to just get SENtral data and send to remote machine over UDP
 */
...
/* EM7180_MPU9250_BMP280_t3 Basic Example Code
 ...
 mode but with er consumption of 20 microAmp, or in a lower resolution mode with er consumption of
...
// wifi vars
//const char* ssid = "ASUS";         // The SSID (name) of the Wi-Fi network you want to connect to
//const char* password = "asusasus";     // The password of the Wi-Fi network
const char* ssid = "dlink-conte";         // The SSID (name) of the Wi-Fi network you want to connect to
const char* password = "conte2018";     // The password of the Wi-Fi network

// UDP Variables
WiFiUDP UDP; // UDP object
IPAddress remoteIp = {192,168,1,236}; // lisa
//IPAddress remoteIp = {192,168,1,49}; // surface
//IPAddress remoteIp = {192,168,1,140}; // JB
unsigned int remotePort = 6000;
unsigned int localPort = 8888;
int sendDelay = 10; // how often to send updates over udp

// Message vars
byte message[10]; // message that gets sent in sendInfo
int YawInt;
int PitchInt;
int RollInt;
int axInt;
int ayInt;
int azInt;
byte lowBitMask = 00000001;
byte bottomFourMask = B00001111;

// warm start
int16_t warm_start = 1; // 0 for no warm start, 1 for warm start
...
int16_t accel_cal = 1; // 0 for no accel cal, 1 for accel cal. accel cal is using the stored accel data like warm start is for magnetometer
...
void sendInfo(float Yaw, float Pitch, float Roll, float ax, float ay, float az);
...
#define AK8963_CNTL      0x0A  // er down (0000), single-measurement (0001), self-test (1000) and Fuse ROM (1111) modes on bits 3:0
...
// Pin definitions
int intPin = 14;  // These can be changed, 2 and 3 are the Arduinos ext int pins - I changed this to 14
int myLed = 15;  // LED on the ESP8266
...
void setup()
{
  ...
  connectWifi();
  boolean udpConnected = connectUDP();
  // loop forever if couldn't connect to UDP
  while(!udpConnected){
    if(SerialDebug){
      Serial.println("no udp");
    }
  }
  if(SerialDebug) Serial.print("udp done");
 ...
 Wire.setClock(400000); // choose 400 kHz I2C rate
  ...
  if(SerialDebug) Serial.println("before I2CScan");
  
  I2Cscan(); // should detect SENtral at 0x28

  if(SerialDebug) Serial.println("after I2CScan");
  ...
  if(SerialDebug) Serial.println("check1");
...
//    if(readByte(EM7180_ADDRESS, EM7180_SentralStatus) & 0x10)  Serial.println("No EEPROM detected!");
  ...
  if(SerialDebug) Serial.println("check2");
   ...
   if(warm_start)
   {
    ...
    if(accel_cal)
    {
      if(SerialDebug) Serial.println("!!!Accel Cal Active!!!");
    ...
    {
      if(SerialDebug) Serial.println("***No Accel Cal***");
    }
   
   delay(1000); // give some time to read the screen
    
  // Set up the SENtral as sensor bus in normal (master) operating mode
  if(!passThru) {
  ...
  {
    if(SerialDebug){
      Serial.println("load accel cal NEW");
    }
  ...
  writeByte(EM7180_ADDRESS, EM7180_HostControl, 0x01); // Force initialize
    ...
    {
      if(SerialDebug){
        Serial.println("load warm start NEW");
      }
 ...
 // Set accel/gyro/mage desired ODR rates
  writeByte(EM7180_ADDRESS, EM7180_QRateDivisor, 0x02); // 100 Hz - can be as high as gyro rate though. might want to set this to 200 later. read "the host side program" for how
  writeByte(EM7180_ADDRESS, EM7180_MagRate, 0x64); // 100 Hz
  writeByte(EM7180_ADDRESS, EM7180_AccelRate, 0x14); // 200/10 Hz. Means it'll be 200 Hz
  writeByte(EM7180_ADDRESS, EM7180_GyroRate, 0x14); // 200/10 Hz. Means it'll be 200 Hz
  writeByte(EM7180_ADDRESS, EM7180_BaroRate, 0x80 | 0x32);  // set enable bit and set Baro rate to 25 Hz (really entering 50 Hz see documentation for why)
 ...
 writeByte(EM7180_ADDRESS, EM7180_EnableEvents, 0x07);
  ...
  if(SerialDebug) Serial.println("Beginning Parameter Adjustments");
  ...
  EM7180_set_mag_acc_FS (0x3E8, 0x08); // 1000 uT, 8 g HERE NEW FULL SCALE ACCEL RANGE
...
//  }
//  }
  if(SerialDebug) Serial.println("end setup");
}
...
//  if(errorStatus == 0x80) Serial.print("Invalid sample rate!");
 ...
 // if no errors, see if new data is ready
  ...
  if(eventStatus & 0x08) { // new mag data available
    ...
    readSENtralQuatData(Quat); // Quat is a 4 elem global array
   }
...
//  } 
 ...
 // keep track of rates
 /*
  * Commented out this part. I think it's only used for the open source sensor fusion stuff
  Now = micros();
...
*/
    ...
    delt_t = millis() - count;
    if (delt_t > sendDelay) { // don't send every loop will crash processing
...
//    //Serial.print(" mz = "); //Serial.print( (int)mz); //Serial.println(" mG");
    ...
    */
    // TEST - will want to send this to processing just don't want to show it for now
...
//   }
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    */
    ...
    Pitch = -asinf(2.0f * (Quat[0] * Quat[2] - Quat[3] * Quat[1]));
...
//    //Serial.print(", ");
//    //Serial.println(Roll, 2);
      sendInfo(Yaw, Pitch, Roll, ax, ay, az);
...
//===================================================================================================================
//====== Set of functions for Wifi and UDP
//===================================================================================================================

/*
 * Connects to Wifi
 * Loop until connection estabilshed
...
/*
 * Receives messages from remote machine
 * Responds with new message
 */
void receiveAndRespond(){
...
/*
 * Sends message to remote machine
 * Input: Yaw: 0 -> 360
 *        Pitch: -90 -> +90
 *        Roll: -180 -> +180
 *        ax: +- 16 up to 2 decimal places
 *        ay: +- 16 up to 2 decimal places
 *        az: +- 16 up to 2 decimal places
 * Accel data is all +-16 even though the accelerometer is only sensitive up to +-8g so I think it's really all double. Doesn't matter will just find usable threshold
 * to send: my encoding is all positive 8 bit bytes
 * message[0]: top 8 bits of Yaw
 * message[1]: Pitch + 90
 * message[2]: top 8 bits of (Roll+180)
 * message[3]: bit 8 is bottom bit of (Roll+180), bit 7 is bottom bit of (Yaw)
 */
void sendInfo(float Yaw, float Pitch, float Roll, float ax, float ay, float az){
    ...
    // convert to ints
    YawInt = round(Yaw);
    PitchInt = round(Pitch) + 90; 
    RollInt = round(Roll) + 180;

    // bit shift and enter angles in message
    message[0] = YawInt >> 1;
    message[1] = PitchInt;
    message[2] = RollInt >> 1;
    message[3] = ((YawInt & lowBitMask) << 1) & (RollInt & lowBitMask);

    // ACCEL DATA
    // multiply all by 100 to keep 2 decimal places, divide by 2 since it's really only +- 8, (ie multiply by 50), now it's +- 800
    // round to make int, add 800 to make positive since sending bytes not ints, now +-1600
    // need 12 bits per axis
    axInt = round((ax*50)) + 800;
    ayInt = round((ay*50)) + 800;
    azInt = round((az*50)) + 800;

    // just put all in message
    message[4] = axInt >> 4; // ax top 8
    message[5] = axInt & bottomFourMask; // ax bottom 4
    message[6] = ayInt >> 4;
    message[7] = ayInt & bottomFourMask;
    message[8] = azInt >> 4;
    message[9] = azInt & bottomFourMask;
    
    UDP.beginPacket(remoteIp, remotePort);
    UDP.write(message,sizeof(message));
    UDP.endPacket();

    // if debug mode also print
    if(SerialDebug){
      Serial.print("Yaw: "); Serial.print(round(Yaw)); Serial.print(" Pitch: "); Serial.print(round(Pitch)); Serial.print(" Roll: "); Serial.print(round(Roll));
      Serial.print(" ax: "); Serial.print(ax); 
      Serial.print(" ay: "); Serial.print(ay);
      Serial.print(" az: "); Serial.println(az);

      // testing bit shifted az
//      Serial.print("mask: "); Serial.println(bottomFourMask);
//      Serial.print(" aztop: "); Serial.print(azInt >> 4); Serial.print( " azbottom: "); Serial.println(azInt & bottomFourMask);
...
// STILL OLD ONE DON'T NEED NEW ONE
       void M24512DFMwriteBytes(uint8_t device_address, uint8_t data_address1, uint8_t data_address2, uint8_t count, uint8_t * dest)
{
  if(count > 128) {
        count = 128;
...
/*
 * Reads in 32 byte chunks. The ESP8266's Wire buffer is only 32 bytes long so need to break it up.
 * The original code was written for the Teensy which has a much bigger buffer.
 * input: device_address is the address of the SENTRAL's memory
 *        data_address1 is the first part of the specific place in the SENTRAL's memory that we want
 *        data_address2 is the second (ie the lower order bits of the address
 *        count is how many bytes to read
 *        dest is the location the data will be placed into
 */
void M24512DFMreadBytes(uint8_t device_address, uint8_t data_address1, uint8_t data_address2, uint8_t count, uint8_t * dest)
{ 
  uint8_t i = 0;
...
//  Wire.endTransmission(I2C_NOSTOP);         // Send the Tx buffer, but send a restart to keep connection alive
    Wire.endTransmission(false);              // Send the Tx buffer, but send a restart to keep connection alive
    Wire.requestFrom(device_address, toRead);       // Read bytes from slave register address 
//  Wire.requestFrom(device_address, (size_t) count);  // Read bytes from slave register address 
    while (Wire.available()) {
      dest[i++] = Wire.read(); 
...
// Copied from ESPWarmStartandAccel_UDP
void EM7180_acc_cal_upload()
{
  int64_t big_cal_num;
  union
